...
<%
    ...
    user user = operateUser.GetUser(articleDate.getUser_id());
    if (request.getParameter("user") == null && !articleDate.getUser_id().equals((String)session.getAttribute("ID"))){
        operateArticle.Click(articleDate.getArticle_id());
    }
...
%>
<head>
    <title><%=articleDate.getArticle_name()%></title>
...
<div id="center">
    <article id="article">
        <div id="article_header">
            <span><%=articleDate.getArticle_name()%></span></br>
            <small>温柔正确的人总是难以生存，因为这世界既不温柔，也不正确</small>
        </div>
        <div id="article_content">
            <textarea disabled id="article_content_text">
                <%=articleDate.getArticle()%>
            </textarea>
        </div>
        <div id="article_footer">
            <p><small>by:<%=user.getUser_name()%></small></p>
            <p><small><%=articleDate.getDate()%></small></p>
        </div>
        <%if (request.getParameter("user") != null){%>
        <p id="button_check">
            <button id="check_pass" value="<%=articleDate.getArticle_id()%>"><strong>通过</strong></button>&nbsp;
            <button id="check_veto" value="<%=articleDate.getArticle_id()%>"><strong>拒绝</strong></button>
        </p>
        <%}%>
...
<script src="${pageContext.request.contextPath}/resources/js/readArticle.js"></script>
<script>
    ...
    window.onload = function (){
        ...
        if (success == "true"){
            layer.alert("发布成功",{ icon: 1, title: "成功", offset: "auto", skin: 'layui-layer-molv' })
        }
    }
</script>
</html>
